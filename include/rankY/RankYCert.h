...
namespace RankYCert
{
   ...
   * algorithm converges q-superlinearly with order (1+theta). */
      ...
      * perform per out iteration */
      ...
      double threshold_min_svd = 1e-05; 
...
class RankYCertClass
{
 ...
 public:
    ...
    RankYCertClass( const RankYCertOptions & options = RankYCertOptions()): options_(options){};        
                           

    ~RankYCertClass(void){};


    RankYCertResult<Matrix, MatrixY, VectorM> getResults(const VectorX & xm,
                                                         const Matrix & costMatrix,
                                                         const std::vector<Matrix> & constMatrices,
                                                         const VectorM & init_mult, 
                                                         const MatrixY & init_Hessian); 

    void printResult(RankYCertResult<Matrix, MatrixY, VectorM> & results);


    bool escape_saddle_rank_y(const VectorX & xm, const Matrix & costMatrix, const std::vector<Matrix> & constMatrices,
                                const VectorM & init_mult, const MatrixY & Yprev, Eigen::MatrixXd & Yplus); 
